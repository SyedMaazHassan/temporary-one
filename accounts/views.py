    ...
    if request.method == 'POST':
        ...
        if form.is_valid():  
            ...
            if user is not None: 
                ...
                if obj.is_active:
                    if obj.role == 'student' or obj.role == 'Instructor':
                        login(request, user)
                        return redirect(f'/user_profile/{obj.id}/')
                    # if obj.role == 'Instructor':
                    #     login(request, user)
                    #     return redirect(f'/instructor/{obj.id}/')
                    if obj.is_admin:
                        login(request, user)
                        if MyUser.objects.get(email=user.email):
                            return redirect(f'/saidatech_admin/{obj.id}/')
                        else:
                            admin_obj=saidatech_admin_profile.objects.create(saidatech_admin_id=user.id)
                            return redirect(f'/saidatech_admin/{obj.id}/')
                    else:
...
def password_change(request):
    ...
    return render(request, "user_templates/user-dashboard.html", context)
...
class RegistrationView(View):
    ...
    def post(self, request):
        ...
        id=uuid.uuid4()
        #let us create this user
        user=MyUser.objects.create(id=id, email=email,role='student',is_active=False)
        user.set_password(password)
        user.save()
        #create a profile for the newly created user
        student_obj=Student_profile.objects.create(student_id=id,fname=fname,lname=lname)
        
        uidb64=urlsafe_base64_encode(force_bytes(user.pk)).decode()
        domain = get_current_site(request).domain
        link=reverse('activate', kwargs={'uidb64':uidb64,'token':generate_token.make_token(user)})
        activate_url='http://'+ domain + link
        print('Please retrive Activation URL: ', activate_url)

        email_body='Hi, please activate your Saidatech Learning account with this link\n' + activate_url
        email_subject='ACTIVATE YOUR SAIDATECH ACCOUNT'
        
        try:
            mail = send_mail(email_subject, email_body, mail_send_from, [email]) #html_message=html_message)
            message = "success"
...
def password_reset(request):
    ...
    if request.method == 'POST':
        ...
        return render(request,'auth/password_reset_form.html',{'form': form,'title':'Password Reset'})
